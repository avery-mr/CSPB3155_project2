...
sealed trait Figure {
    ...
    def rotate(radians: Double): Figure
...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) = {
        val x_list = cList.map(_._1)
        val y_list = cList.map(_._2)

        (x_list.min, x_list.max, y_list.min, y_list.max)
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon = {
        val cList_shifted = cList.map {
            case (x, y) => (x + shiftX, y + shiftY)
        }
        Polygon(cList_shifted)
    }

    // Function: rotate -- rotate each point of polygon around (0,0) in radians
    override def rotate(radians: Double): Polygon = {
        def rotate_point(x: Double, y:Double, radians: Double): (Double, Double) = {
            val x_new = x * math.cos(radians) - y * math.sin(radians)
            val y_new = x * math.sin(radians) + y * math.cos(radians)
            (x_new, y_new)
        }

        val cList_rotated = cList.map { case (x,y) => rotate_point(x, y, radians)}

        Polygon(cList_rotated)
    }
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): MyCircle = {
        val center_shifted = (c._1 + shiftX, c._2 + shiftY)
        MyCircle(center_shifted, r)
    }

    // Function: rotate -- rotate each point of polygon around (0,0) in radians
    override def rotate(radians: Double): MyCircle = {
        
        val new_x = c._1 * math.cos(radians) - c._2 * math.sin(radians)
        val new_y = c._1 * math.sin(radians) + c._2 * math.cos(radians)
        MyCircle((new_x, new_y), r)
    }
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
        val boundingBoxes = listOfObjects.map(_.getBoundingBox)
        //println(boundingBoxes)
        //(1.0, 1.0, 1.0, 1.0)

        val x_min = boundingBoxes.map(_._1).min
        val x_max = boundingBoxes.map(_._2).max
        val y_min = boundingBoxes.map(_._3).min
        val y_max = boundingBoxes.map(_._4).max
    ...
    def translate(shiftX: Double, shiftY: Double): MyCanvas = {
        val translated_objects = listOfObjects.map(obj => obj.translate(shiftX, shiftY))
        new MyCanvas(translated_objects)
    }
    ...
    def placeRight(myc2: MyCanvas):MyCanvas = {
        val box_c1 = this.getBoundingBox
        val box_c2 = myc2.getBoundingBox
        val xShift = box_c1._2 - box_c1._1
        val yShift = (box_c1._4 - box_c1._3) / 2 - (box_c2._4 - box_c2._3) / 2
        val c2_shifted = myc2.translate(xShift, yShift)
        overlap(c2_shifted)
    ...
    def placeTop(myc2: MyCanvas): MyCanvas = {
        val box_c1 = this.getBoundingBox
        val box_c2 = myc2.getBoundingBox
        val xShift = (box_c1._2 - box_c1._1) / 2 - (box_c2._2 - box_c2._1) / 2
        val yShift = box_c1._4 - box_c1._3
    ...
    def rotate(angRad: Double): MyCanvas = {
        val rotated_objects = listOfObjects.map(obj => obj.rotate(angRad))
        new MyCanvas(rotated_objects)
    }
